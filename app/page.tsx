...
export default function Home() {
  ...
  // Custom sequence helper functions
  const addToSequence = (part: CustomBodyPart) => {
    setSettings({
      ...settings,
      customSequence: [...(settings.customSequence || []), part],
    });
  };

  const removeFromSequence = (index: number) => {
    const newSequence = [...(settings.customSequence || [])];
          ...
          <div>
            ...
            {settings.customSequence && settings.customSequence.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                <div className="text-xs text-gray-600 mb-2">선택된 순서:</div>
